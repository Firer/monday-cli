...
import { emitDryRun, emitSuccess } from '../emit.js';
...
interface BoardLookupResponse {
  readonly items: readonly { readonly id: string; readonly board: { readonly id: string } | null }[] | null;
}
...
const resolveBoardId = async (
  ...
  const response = await client.raw<BoardLookupResponse>(
 ...
 * thrown ApiError's details.resolver_warnings slot — same pattern the
 * dry-run engine uses on `column_archived` so a stale-cache-then-
 * archived flow doesn't drop the refresh signal. The function returns
 * a NEW ApiError with merged details; the original is discarded.
 */
    ...
    noun
      ...
      .action(async (itemId: unknown, setExpr: unknown, opts: unknown) => {
        ...
        try {
          ...
          // Translator errors after resolution succeeded — fold any
          // resolver warnings (collision / stale_cache_refreshed) into
          // details.resolver_warnings so the error envelope preserves
          // the refresh / collision signal. Same shape the dry-run
          // engine uses on column_archived.
        ...
        const mutationResult = await executeMutation(client, {
          mutation,
          itemId: parsed.itemId,
          boardId,
        });
