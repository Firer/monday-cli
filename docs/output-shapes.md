...
### `user team-*` writer surface (v0.5-M34)

The six v0.5-M34 team-writer verbs (`team-list` / `team-get` /
`team-create` / `team-delete` / `team-add-members` /
`team-remove-members`) ship full envelope shapes documented at
[`docs/cli-design.md`](./cli-design.md) §4.3 USER row + pinned by
the per-verb integration tests at
`tests/integration/commands/team-*.test.ts` (6 files, 40
behavioural cases) + the envelope-snapshot regression suite
(12 snapshots at `tests/integration/envelope-snapshots.test.ts`
under the M34 team-writers describe block). Per-verb sections
in this doc deferred — caught at v0.5 release-prep ToC audit
as a v0.5-M34 close-docs gap; full sections land at v0.5.x as
a documentation backfill.
...
`details.deferred_to: "v0.6"` (M28 Decision 11 closure — Monday's
...
because neither v0.4 nor v0.5 picked the feature up and the
data-model gap is unchanged).
...
### `monday doc create-in-workspace --workspace <wid> --name <n> [--folder <fid>] [--kind public|private|share]` (v0.5-M35)

Create a new workspace-scoped workdoc via Monday's
`Mutation.create_doc(location: { workspace: { workspace_id!,
name!, folder_id?, kind? } }) → Document` (operationName
`CreateDocInWorkspace`). Splits Monday's mutually-exclusive
`CreateDocInput.board` vs `CreateDocInput.workspace` shape into
two distinct CLI verbs per D7 closure — the mutual-exclusion
...
**Dry-run envelope** per cli-design §6.4:
...
### `monday doc create-on-column --item <iid> --column <cid>` (v0.5-M35)

Create an item-scoped workdoc embedded into a doc-column via
Monday's `Mutation.create_doc(location: { board: { column_id!,
item_id! } }) → Document` (operationName `CreateDocOnColumn`).
...
on the item's board; CLI does NOT pre-check column-type
...
### `monday doc rename <did> --name <n>` (v0.5-M35)

Rename an existing workdoc via Monday's
`Mutation.update_doc_name(docId: ID!, name: String!) → JSON`
(operationName `UpdateDocName`). Wire return is the opaque
`JSON` scalar — the CLI projects to flat `{ doc_id: <echoed>,
...
Idempotent: yes (rename converges to a stable name; Monday's
wire is no-op when name matches current value).

### `monday doc delete <did> --yes` (v0.5-M35)

Delete an existing workdoc via Monday's
`Mutation.delete_doc(docId: ID!) → JSON` (operationName
`DeleteDoc`). Wire return is the opaque `JSON` scalar —
projected to flat `{ doc_id: <echoed>, success: true }` per
D9.

**Destructive — `--yes` required outside `--dry-run` per
§3.1 #7.** Gate fires BEFORE `resolveClient` per M10
round-1 P2 invariant (a missing token never masks
...
**Missing `--yes` envelope** (`confirmation_required` per
§6.5):
...
```json
{
  ...
  "error": {
    "code": "confirmation_required",
    "message": "monday doc delete 12345678 would delete the workdoc — pass --yes to confirm, or --dry-run to preview.",
    "details": {
      "doc_id": "12345678",
      "hint": "delete is destructive — Monday's wire surface offers no restore mutation for workdocs; agents needing reversal must recreate via `monday doc create-in-workspace` / `create-on-column` (lossy: new id, content must be re-imported)."
    },
    "http_status": null,
    "monday_code": null,
    "request_id": null,
...
**Dry-run envelope** (destructive-no-read pattern):
...
**Null wire payload** surfaces `not_found` with
`details.doc_id` (mirrors M14 `workspace delete` + M34 `team
delete` cadence — id was bogus or doc already deleted by a
concurrent caller).

Idempotent: no (re-running surfaces `not_found` past the first
call).

### `monday doc duplicate <did> [--with-updates]` (v0.5-M35)

Duplicate an existing workdoc via Monday's
`Mutation.duplicate_doc(docId: ID!, duplicateType?:
DuplicateType) → JSON` (operationName `DuplicateDoc`).
Projects Monday's opaque JSON return into flat `{ doc_id:
<NEW>, success: true }` per D9 — **`doc_id` carries the
NEWLY-CREATED duplicate's id**, NOT the source-doc positional.

**`--with-updates` semantics.** Maps the boolean argv to
Monday's `DuplicateType` 2-value wire enum: absent → wire-
side default `duplicate_doc_with_content` (content-only);
present →
`duplicate_doc_with_content_and_updates` (clone body + every
comment / update thread).

**No `--name <n>` slot per D8 closure** — Monday's
`duplicate_doc` carries no rename-on-duplicate arg; the
duplicate inherits Monday's auto-generated copy name. Agents
needing a renamed duplicate pair with `monday doc rename
<new-id> --name <n>`.
...
**Dry-run envelope** (source `doc_id` echoed; new-id only
available on live):
...
`{success: false, error: <message>}` envelope. The CLI's projection
contract (D12 closure) is pinned by M37 IMPL integration cassettes
at `tests/integration/commands/doc-{import-html,append-markdown}.
test.ts`: any populated wire `error` string surfaces verbatim under
`validation_failed.details.error`; null/empty `error` strings on
`success: false` surface as `internal_error` with a wire-regression
hint.

The literal wire `error` strings Monday emits for each failure mode
are NOT pinned by introspection — they're free-form English text
generated by Monday's server-side rejection logic. M37 IMPL
integration cassettes exercise the STRUCTURAL contract (the
projection mapping) using synthetic-but-realistic `error` strings.
Cells marked `cassette-pinned` carry the structural contract from
the IMPL test file; cells marked `TBD (v0.5.x real-wire probe)`
await a live cassette to pin Monday's verbatim wire text for
display purposes.

| Mutation | Failure source | Wire `error` string | Mapped CLI envelope |
|----------|----------------|---------------------|---------------------|
| `import_doc_from_html` | Invalid HTML payload (malformed tags / broken structure) | cassette-pinned structural contract via `doc-import-html.test.ts` (synthetic: `"invalid HTML structure at line 12"`); literal Monday text TBD (v0.5.x real-wire probe) | `validation_failed.details: {workspace_id, error: <verbatim>, hint}` |
| `import_doc_from_html` | Workspace lacks doc-create permission | TBD (v0.5.x real-wire probe) | `validation_failed.details.error: <verbatim>` OR `forbidden` per transport-layer mapping |
| `import_doc_from_html` | Wire regression: `success: false + null/empty error` | cassette-pinned via `doc-import-html.test.ts` (`null` and `""` both covered) | `internal_error.details: {workspace_id, hint: 'wire-shape regression — ...'}` |
| `import_doc_from_html` | Wire regression: `success: true + null doc_id` | cassette-pinned via `doc-import-html.test.ts` | `internal_error.details: {workspace_id, hint: 'wire-shape regression — ...'}` |
| `import_doc_from_html` | Oversized inline payload (parse-boundary pre-empt) | n/a — caught by schema `.refine()` before wire dispatch | `usage_error.details.issues[{path: 'htmlString', message: '...exceeds the 256000-byte wire-side limit ...'}]` (D13) |
| `import_doc_from_html` | Oversized file payload (runtime pre-empt) | n/a — caught by `readSourceContent.maxBytes` at file/stdin read boundary | `usage_error.details: {source: 'file'\|'stdin', size_bytes, limit_bytes, file_path?}` (D13 IMPL) |
| `import_doc_from_html` | Oversized payload that slipped past parse + runtime guards | `Internal Server Error` (`extensions.code: INTERNAL_SERVER_ERROR`) | `internal_error` (transport-layer fallback; should never fire — `MAX_DOC_IMPORT_PAYLOAD_BYTES = 256_000` keeps payloads safely under the 500KB-rejected threshold) |
| `add_content_to_doc_from_markdown` | Invalid markdown payload (rare — markdown is permissive) | cassette-pinned structural contract via `doc-append-markdown.test.ts` (synthetic: `"markdown parse failure: unmatched fence at line 3"`); literal Monday text TBD (v0.5.x real-wire probe) | `validation_failed.details: {doc_id, error: <verbatim>, hint}` |
| `add_content_to_doc_from_markdown` | Non-existent / inaccessible `<doc-id>` | TBD (v0.5.x real-wire probe) | `not_found` (likely; transport-layer mapping) OR `validation_failed` per wire shape |
| `add_content_to_doc_from_markdown` | `afterBlockId` references a block in a different doc | TBD (v0.5.x real-wire probe) | `validation_failed.details.error: <verbatim>` |
...
`TBD (v0.5.x real-wire probe)` await a live cassette against Monday
to pin verbatim wire-side rejection strings for display purposes
(the CLI's projection contract is structural — verbatim `error`
text surfaces under `validation_failed.details.error` regardless of
the literal string). Extending the table is additive at any
future v0.5.x patch as new failure modes surface live.
