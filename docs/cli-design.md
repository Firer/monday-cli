...
- **Carried forward from v0.4 + v0.5 release-prep slips**
  ...
  - `<path>='-'` stdin for file `--set` — deferred from M38 per
    D7 → **✅ SHIPPED at v0.8-M47 IMPL (`0d99ea7`; Codex CONVERGED R1)**.
    The argv `<file-col>=-` + `--filename` surface + the
    `routeFileColumnDispatch` stdin-scope enforcement landed at the
    pre-flight; the IMPL swapped the stub for the live stdin-read +
    Blob → `add_file_to_column` leg + the D4 size-less dry-run echo (an
    empty pipe rejects `usage_error` `stdin_file_empty`). Scoped to
    single-file,
